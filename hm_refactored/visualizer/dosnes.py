...
class DOSNES():
    ...
    def _fit(self, X, render_name='Temp', to_render=False, render_y=None, plt_cmap=plt.cm.Set1):
        """Apply Sinkhorn–Knopp Algorithm to have a Doubly Stochastic Matrix.
        Parameters
        ----------
        ...
        for iter in range(self.max_iter):
            ...
            if iter % self.verbose_freq == 0:
                ...
                if to_render:
                    self._render(iter, cost, render_name, render_y, plt_cmap)

        self.embedding = ydata / np.sqrt(np.sum(ydata ** 2, axis=1)).reshape(-1, 1)

        if to_render:
            self._make_gif(render_name, 'progress.gif')
    ...
    def fit_transform(self, X, render_name = 'Temp', to_render = False, render_y = None, plt_cmap=plt.cm.Set1):
        ...
        """
        self._fit(X, render_name, to_render, render_y=render_y, plt_cmap=plt_cmap)
    ...
    def fit(self, X, y = None):
        ...
        y : array, shape (n_samples, 1)
            Classes used for color in rendering
        """
        self.fit_transform(X, render_y=y)
        return self

    def _render(self, iter, cost, render_name, render_y, plt_cmap):
        progress_path = os.path.join(self.data_path, render_name)
        if not os.path.isdir(progress_path):
            os.makedirs(progress_path)

        fig = plt.figure()
        ax = fig.gca(projection='3d')
        if render_y is not None:
            ax.scatter(self.embedding[:, 0], self.embedding[:, 1], self.embedding[:, 2], c=render_y, cmap=plt_cmap)
        else:
        ...
        plt.savefig(os.path.join(progress_path, "iter{0:03d}.png".format(iter)))
        plt.close('all')

    def _make_gif(self, render_name, filename):
        progress_path = os.path.join(self.data_path, render_name)
        images = []
        import imageio
        # for img in glob.glob("temp/iter*.png"):
        for img in glob.glob(progress_path + "/iter*.png"):
            images.append(imageio.imread(img))
        imageio.mimsave(self.data_path + '/' + '{}_{}'.format(render_name, filename), images)
