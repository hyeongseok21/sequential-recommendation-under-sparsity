...
a real fashion purchase dataset을 바탕으로 sparse interaction regime에서의 sequential recommendation을 분석합니다. 핵심 질문은 item metadata가 recommendation quality를 개선하는지, 그리고 어떤 user regime에서 그 효과가 가장 크게 나타나는지입니다.

이 결과는 extreme sparsity 환경에서는 model architecture만으로 recommendation quality를 끌어올리기 어렵고, metadata나 더 풍부한 behavioral history 같은 stronger signal이 필요하다는 점을 보여줍니다.

## Main Findings

- `TopPopular`는 extreme sparsity 환경에서 여전히 가장 강한 overall baseline이었습니다.
- personalized model 중에서는 `DIF-SR + Metadata`가 가장 높은 성능을 보였습니다.
- Metadata는 cold-like user와 short-history user처럼 weak behavioral signal 환경에서 가장 유용했습니다.
- Metadata는 short-history user(`≤8` interactions) 중 recent `product_type` switch가 `1-2회` 나타나는 구간에서 가장 큰 이득을 보였고, 이때 ranking quality와 candidate frontier 모두에서 개선이 확인됐습니다.
...
| sparse interaction regime에서 backbone 차이는 어떻게 나타나는가? | model architecture 차이가 dataset regime 효과와 어떻게 상호작용하는지 해석합니다. |
...
## Metadata and Head Selection

최종 설정에 이르기까지 metadata weighting과 `n_heads`도 함께 비교했습니다. 어떤 조합이 짧은 예산의 fast-scout 신호를 넘어 full validation까지 안정적으로 유지되는지 확인했습니다.

<img src="plots/metadata_head_selection.png" alt="Metadata and head selection" width="720" />

`product_type` weighting은 tested metadata adjustment 중 유일하게 full validation까지 유지되며 최종 champion으로 남았습니다. 반면 `h4`는 fast-scout에서는 가장 좋아 보였지만 full validation에서는 champion을 넘지 못해, 이 데이터셋에서는 더 넓은 head setting보다 더 compact한 final configuration이 안정적으로 작동했다고 해석할 수 있습니다.

**한 줄 해석:** metadata selection에서는 `product_type`가 가장 안정적이었고, head selection에서는 scout signal보다 full-validation stability가 최종 선택을 결정했습니다.

## Candidate Frontier Analysis
...
Service-style slice를 기준으로, 메인 user regime에서 어떤 모델이 우세한지 비교했습니다.
...
<img src="plots/final_slice_analysis.png" alt="Slice analysis summary" width="720" />
...
보조 분석으로 `high-entropy`와 `switch-heavy` user도 추가 확인했지만, 이 구간은 mixed-intent 신호와 긴 history가 강하게 함께 나타나 메인 regime summary와는 분리해 해석하는 편이 더 적절했습니다.

### Metadata Gain under Structured Intent Shift

최근 `product_type` switching을 기준으로 mixed-intent를 다시 정의해, short-history regime에서 metadata 효과를 추가로 확인했습니다.

특히 `history ≤ 8` 이고 recent `product_type` switch가 `1-2회`인 구간에서는 `DIF-SR + Metadata`가 `Corrected SASRec`보다 `Recall@20`과 `NDCG@20` 모두에서 우세했습니다. 이 결과는 non-repeat filtering에서도 유지됐습니다.

세부적으로는 `1 switch` 구간에서 metadata gain이 가장 강했고, `2 switches`로 늘어나면 `Corrected SASRec`의 경쟁력이 다시 커졌습니다. 또한 recent sequence가 dense할수록 metadata gain이 강화됐습니다.

이 구간의 candidate overlap도 매우 낮아, 개선이 단순 reranking이 아니라 candidate frontier 자체의 변화와 함께 나타났음을 확인했습니다.
...
| Structured recent intent shift | `DIF-SR + Metadata`가 short-history 구간에서 더 강함 | metadata is most effective when weak behavioral signals still preserve coherent short-term structure |
| Strong popularity bias | `TopPopular`가 전체적으로 가장 강함 | global popularity dominates personalized signal in this dataset |
| Repeat-heavy scenario | `Corrected SASRec`가 강함 | this regime behaves more like sequence memorization than sparse discovery |

복잡한 model architecture만으로는 extreme sparsity를 쉽게 극복하기 어렵다는 점을 보여줍니다. 실제 개선에는 metadata signal, 더 풍부한 behavioral history, 도메인 특화 recommendation policy가 함께 필요합니다.
...
| `Phase 3: Baseline Sanity Check` | anomaly detection, causal masking issue discovery | 잘못된 baseline을 찾아냈고, 이후 모델 비교 해석을 바꾸는 분기점이 됩니다. |
...
Service-style supplementary evaluation report 생성:
...
최종 portfolio plots 및 curated reports 생성:
