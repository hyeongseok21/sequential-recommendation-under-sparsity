...
초기 SASRec baseline은 causal masking 누락과 지나치게 약한 학습 설정 때문에 신뢰하기 어려웠습니다.
...
| Training setup | undertrained configuration | stronger sanity configuration | underfitting 때문에 왜곡된 비교를 줄입니다. |
...
`TopPopular`가 전체적으로 가장 강한 모델로 남았다는 점은 이 데이터셋의 세 가지 구조적 특성으로 설명할 수 있습니다.
