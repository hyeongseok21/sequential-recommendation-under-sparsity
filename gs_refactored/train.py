...
from gs_preprocess import gs_prep
...
class Trainer:
    def __init__(self, config_path, config=None):
        ...
        if not os.path.isfile(dataset_path): 
            ...
            self.data_dict = gs_prep(self.dataset_params)
        else:
        ...
        if self.eval_params['popular_ten']:
            item_count = collections.Counter(self.data_dict['train_df']['item_id'].to_numpy()) # item_id별 count를 내림차순의 dict형태로 변환
            
            def invert_dict(d):
                result = collections.defaultdict(set) # key가 없으면 default로 set을 반환하는 dictionary
                for k in d:
                    result[d[k]].add(k) # d[k]를 key, k를 value로 가지는 element를 result에 추가
                return dict(result) # result를 dict 형태로 변환하여 반환
            
            item_count_inv = invert_dict(item_count)
            popular_items_count = sorted(list(item_count_inv.keys()), reverse=True)[:self.train_params['top_k']] # count들을 내림차순으로 정렬하고, 상위 top_k개 만큼의 item_count list 생성
            print("popular_items_count:", popular_items_count)
            self.popular_items = []
            for per_item_count in popular_items_count:
                if len(self.popular_items) > self.train_params['top_k']:
                    break
                # self.popular_items에 item_id list를 top_k개가 될때까지 채워넣음
                self.popular_items = self.popular_items + list(item_count_inv[per_item_count])[:(self.train_params['top_k'] - len(self.popular_items))]
            print("self.popular_items:", self.popular_items)
    ...
    def train(self, use_checkpoint=False):
        ...
        for e in range(start_epoch, start_epoch + self.train_params['train_epoch']):
            ...
            for idx, pos_neg_pair in enumerate(tqdm(self.train_dl)):
                ...
                # train DataLoader로부터 model에 input으로 넣어주는 vector 계산
                ...
                # sampler가 TwoView인 경우 neg_history, neg_history_mask 계산
                if self.model_params['sampler_type'] == "TwoView":
                    neg_history = pos_neg_pair[:, 2+2*self.model_params['seq_len']:2+3*self.model_params['seq_len']]
                    neg_history_mask = pos_neg_pair[:, 2+3*self.model_params['seq_len']:2+4*self.model_params['seq_len']]
